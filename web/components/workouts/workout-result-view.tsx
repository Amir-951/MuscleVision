...
export function WorkoutResultView({sessionId}: {sessionId: string}) {
  ...
  const stats = [
    {label: 'Score', value: formatScore10From100(result.correctnessScore)},
    {label: 'Tempo', value: result.tempo},
    {label: 'Symétrie', value: formatScore10FromUnit(result.symmetryScore)},
    {label: 'Stabilité', value: formatScore10FromUnit(result.stabilityScore)},
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        <LabCard className="space-y-6 p-6">
        ...
        <LabCard className="overflow-hidden p-0">
          <div className="border-b border-white/10 px-6 py-4">
            <div className="flex items-center gap-2 text-xs uppercase tracking-[0.28em] text-mist/45">
              <Orbit className="h-4 w-4 text-amber" />
              Replay anatomique
            </div>
            <p className="mt-3 max-w-xl text-sm text-mist/58">
              Lis le geste, ralentis-le et observe directement les zones qui chauffent.
            </p>
          </div>
          ...
          <div className="p-6">
            <WorkoutMotionReplay
              ...
              viewportClassName="h-[560px] w-full xl:h-[760px]"
            />
          </div>
        </LabCard>
      </div>
